...
# Execute une requete toutes les 2 minutes pour eviter que le modele ne se mette en veille
...
Write-Host "Appuyez sur Ctrl+C pour arreter le script" -ForegroundColor Cyan
Write-Host ""
...
while ($true) {
    try {
        ...
        Write-Host "[$Timestamp] Requete #$RequestCount - Envoi du ping..." -ForegroundColor White
        
        # Execution de la requete
        $Response = Invoke-WebRequest -Uri $Uri -Method POST -Headers $Headers -Body $Body -TimeoutSec 30
        
        if ($Response.StatusCode -eq 200) {
            Write-Host "[$Timestamp] Succes - Modele maintenu actif" -ForegroundColor Green
            
            # Optionnel: afficher une partie de la reponse
